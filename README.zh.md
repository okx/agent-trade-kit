...
| | |
|---|---|
| **118 个工具，7 大模块** | 完整交易生命周期：行情 → 下单 → 算法单 → 账户管理 → 交易机器人 |
...
| `bot` | 46 | 交易机器人：网格（现货/合约/Moon）、现货 DCA、合约 DCA、定期投资。完整生命周期：创建、管理、停止 | [→](docs/modules/bot.md) |
...
okx-trade-mcp --profile live --modules all           # 所有模块，指定 profile
