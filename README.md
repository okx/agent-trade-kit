...
| | |
|---|---|
| **45 tools across 5 modules** | Full trading lifecycle: market data → orders → algo orders → account management |
...
| `account` | 11 | Balance, bills (+archive), positions, positions history, fee rates, config, position mode |
...
| | |
|---|---|
| **45 个工具，5 大模块** | 完整交易生命周期：行情 → 下单 → 算法单 → 账户管理 |
...
| `account` | 11 | 余额、账单（含归档）、持仓、持仓历史、手续费率、配置、仓位模式 |
