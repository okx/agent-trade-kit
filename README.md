...
| | |
|---|---|
| **56 tools across 6 modules** | Full trading lifecycle: market data → orders → algo orders → account management → trading bots |
...
| `account` | 13 | Balance, bills (+archive), positions, positions history, fee rates, config, position mode, max withdrawal, max avail size |
...
| | |
|---|---|
| **56 个工具，6 大模块** | 完整交易生命周期：行情 → 下单 → 算法单 → 账户管理 → 交易机器人 |
...
| `account` | 13 | 余额、账单（含归档）、持仓、持仓历史、手续费率、配置、仓位模式、最大可提币量、最大可用仓位 |
