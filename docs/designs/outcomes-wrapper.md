...
- Package or distribute the `okx-outcomes` binary itself. Distribution is owned by the outcomes team via the `install.sh` + GitHub Releases tarball pattern at `github.com/okx/outcomes`.

## Naming conventions

| Layer | Name |
|---|---|
| Distribution channel | `github.com/okx/outcomes` (`install.sh` + prebuilt {darwin,linux} {x64,arm64} tarballs from GitHub Releases) |
...
- **Binary-not-found UX**: print a friendly install hint (`curl -fsSL https://raw.githubusercontent.com/okx/outcomes/master/install.sh | sh` for macOS/Linux; download `okx-outcomes.exe` from GitHub Releases for Windows) and exit `127`.
...
OKX Outcomes uses **three independent** authentication paths, none of which intersect with the main `okx` CLI's OAuth / API-key flow:
...
> The env-var prefix `PREDICTIONS_` is legacy from the product's previous name; it is preserved by upstream for backwards compatibility.
...
- Upstream repo: <https://github.com/okx/outcomes>
