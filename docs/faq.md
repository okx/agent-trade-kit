...
```bash
agent-tradekit-mcp --modules market
```
...
agent-tradekit-mcp --profile live --modules market spot account
```
...
### `agent-tradekit-mcp` command not found after `npm install -g`
...
```bash
which agent-tradekit-mcp
agent-tradekit-mcp --help
```
...
**Cause**: Claude Desktop uses the system Node.js (typically `/usr/local/bin/node` or `/usr/bin/node`), which is different from the version managed by nvm/fnm in your terminal. If `agent-tradekit-mcp` was installed via the terminal's npm, the system Node environment used by Claude Desktop cannot find the package.
...
```bash
/usr/local/bin/npm install -g agent-tradekit-mcp
```
...
**原因**：Claude Desktop 使用的是系统 Node.js（通常是 `/usr/local/bin/node` 或 `/usr/bin/node`），与终端里通过 nvm/fnm 管理的版本不同。如果 `agent-tradekit-mcp` 是用终端的 npm 安装的，Claude Desktop 对应的 Node 环境里找不到这个包。
