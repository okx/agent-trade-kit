...
> **smartmoney over 5–8 tool budget — approved exception:** the surface is split by entry mode (`_by_filter` / `_by_trader`) so AI agents can disambiguate from the tool name alone. Per-mode atomicity is the minimum granular set; further consolidation re-introduces the multi-mode footgun fixed in 2026-04-29 redesign (see `docs/designs/smartmoney.md` §2). 2026-04-30 net: +`smartmoney_search_trader` (nickname → authorId resolver), −`smartmoney_get_top_coin_signals` (subsumed by `signal_overview_by_filter` with `topInstruments` default). Module is still well under the per-module ≤25,000 budget (~4,729) and shares `outputSchema` fragments via `TRADER_ITEM_PROPS` / `SIGNAL_ITEM_PROPS` to keep growth amortized.
