...
show_next_steps() {
  ...
  echo ""
  warn "⚠️  Security Tips: NEVER send API keys in agent chat. Create a dedicated sub-account for your agent. Test on demo before going live."
  warn "⚠️  安全提示：切勿在Agent对话中发送API Key。请创建Agent专用子账户接入。先在模拟盘充分测试，再接入实盘。"
  echo ""
