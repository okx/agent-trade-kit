...
function Show-NextSteps {
    ...
    Write-Host ""
    $warnEmoji = "$([char]0x26A0)$([char]0xFE0F)"
    Write-Warn "$warnEmoji  Security Tips: NEVER send API keys in agent chat. Create a dedicated sub-account for your agent. Test on demo before going live."
    Write-Warn "$warnEmoji  安全提示：切勿在Agent对话中发送API Key。请创建Agent专用子账户接入。先在模拟盘充分测试，再接入实盘。"
    Write-Host ""
