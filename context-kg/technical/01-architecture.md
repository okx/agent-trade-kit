...
- `packages/cli/test/` — CLI parameter routing and integration tests (43 files), including bidirectional drift test (`drift.test.ts`) that verifies CLI registry ↔ ToolSpec alignment, and context-kg accuracy test (`context-kg-accuracy.test.ts`) that verifies documentation numbers stay in sync with code
