...
- `grid_get_liquidate_price` — estimate liquidation price given instId, sz, and lever; optional direction/basePos/maxPx/minPx/gridNum/runType/triggerStrategy refine the estimate to a specific intended grid shape/entry (read)
- `grid_close_position` — close remaining position after stopType=`"2"` stop (write); mktClose=true for market, false for limit (provide sz and px); use `grid_get_positions` first to confirm the position exists
