...
import { registerRecurringTools } from "../src/tools/bot/recurring.js";
...
describe("dca_create_order", () => {
  ...
  it("triggerCond enum only allows cross_up and cross_down", () => {
    const props = tool.inputSchema.properties as Record<string, { enum?: string[] }>;
    const allowed = props["triggerCond"]!.enum!;
    assert.deepStrictEqual(allowed, ["cross_up", "cross_down"]);
  });
...
describe("recurring tools registration", () => {
  const tools = registerRecurringTools();

  it("registers 6 tools", () => {
    assert.equal(tools.length, 6);
  });

  it("all tools have module=bot.recurring", () => {
    for (const t of tools) {
      assert.equal(t.module, "bot.recurring");
    }
  });

  it("write tools are marked isWrite=true", () => {
    const writeTools = tools.filter((t) => t.isWrite);
    const writeNames = writeTools.map((t) => t.name).sort();
    assert.deepEqual(writeNames, ["recurring_amend_order", "recurring_create_order", "recurring_stop_order"]);
  });

  it("read tools are marked isWrite=false", () => {
    const readTools = tools.filter((t) => !t.isWrite);
    const readNames = readTools.map((t) => t.name).sort();
    assert.deepEqual(readNames, ["recurring_get_order_details", "recurring_get_orders", "recurring_get_sub_orders"]);
  });
});

describe("recurring_create_order", () => {
  const tools = registerRecurringTools();
  const tool = tools.find((t) => t.name === "recurring_create_order")!;

  it("calls /recurring/order-algo via POST", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({
    ...
    assert.equal(getLastCall()?.endpoint, "/api/v5/tradingBot/recurring/order-algo");
    assert.equal(getLastCall()?.method, "POST");
  });

  it("parses recurringList JSON and passes as array", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({
      stgyName: "My DCA", recurringList: '[{"ccy":"BTC","ratio":"0.5"},{"ccy":"ETH","ratio":"0.5"}]',
      period: "daily", recurringDay: "1", recurringTime: "08:00",
      timeZone: "8", amt: "100", investmentCcy: "USDT", tdMode: "cross",
    }, makeContext(client));
    const params = getLastCall()?.params as Record<string, unknown>;
    assert.ok(Array.isArray(params.recurringList));
    assert.equal((params.recurringList as unknown[]).length, 2);
  });

  it("injects sourceTag as tag", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({
      stgyName: "My DCA", recurringList: '[{"ccy":"BTC","ratio":"1"}]',
      period: "daily", recurringDay: "1", recurringTime: "08:00",
  ...
  it("throws on invalid recurringList JSON", async () => {
    ...
    await assert.rejects(
      () => tool.handler({
        stgyName: "My DCA", recurringList: "not-json",
        period: "daily", recurringDay: "1", recurringTime: "08:00",
        timeZone: "8", amt: "100", investmentCcy: "USDT", tdMode: "cross",
      }, makeContext(client)),
  ...
  it("throws on empty recurringList array", async () => {
    const { client } = makeMockClient();
    await assert.rejects(
      () => tool.handler({
        stgyName: "My DCA", recurringList: "[]",
        period: "daily", recurringDay: "1", recurringTime: "08:00",
        timeZone: "8", amt: "100", investmentCcy: "USDT", tdMode: "cross",
      }, makeContext(client)),
  ...
  it("throws when period=hourly but recurringHour is missing", async () => {
    const { client } = makeMockClient();
    await assert.rejects(
      () => tool.handler({
        stgyName: "My DCA", recurringList: '[{"ccy":"BTC","ratio":"1"}]',
        period: "hourly", recurringDay: "1", recurringTime: "08:00",
        timeZone: "8", amt: "100", investmentCcy: "USDT", tdMode: "cross",
      }, makeContext(client)),
      { message: /recurringHour is required/ },
    );
  });

  it("passes recurringHour when period=hourly", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({
      stgyName: "My DCA", recurringList: '[{"ccy":"BTC","ratio":"1"}]',
      period: "hourly", recurringDay: "1", recurringTime: "08:00", recurringHour: "4",
      timeZone: "8", amt: "100", investmentCcy: "USDT", tdMode: "cross",
    }, makeContext(client));
    assert.equal((getLastCall()?.params as Record<string, unknown>).recurringHour, "4");
  });

  it("passes optional algoClOrdId when provided", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({
      stgyName: "My DCA", recurringList: '[{"ccy":"BTC","ratio":"1"}]',
      period: "daily", recurringDay: "1", recurringTime: "08:00",
      timeZone: "8", amt: "100", investmentCcy: "USDT", tdMode: "cross",
      algoClOrdId: "my-custom-id",
    }, makeContext(client));
    assert.equal((getLastCall()?.params as Record<string, unknown>).algoClOrdId, "my-custom-id");
  });

  it("recurringDay is optional in inputSchema (only required for weekly/monthly)", () => {
    const required = tool.inputSchema.required as string[];
    assert.ok(!required.includes("recurringDay"));
  });

  it("passes tradeQuoteCcy when provided", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({
      stgyName: "BTC DCA", recurringList: '[{"ccy":"BTC","ratio":"1"}]',
      period: "daily", recurringTime: "8",
      timeZone: "8", amt: "100", investmentCcy: "USDT", tdMode: "cross",
      tradeQuoteCcy: "USDT",
    }, makeContext(client));
    assert.equal((getLastCall()?.params as Record<string, unknown>).tradeQuoteCcy, "USDT");
  });
});

describe("recurring_amend_order", () => {
  const tools = registerRecurringTools();
  const tool = tools.find((t) => t.name === "recurring_amend_order")!;

  it("calls /recurring/amend-order-algo via POST with algoId and stgyName only", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({ algoId: "123", stgyName: "New Name" }, makeContext(client));
    assert.equal(getLastCall()?.endpoint, "/api/v5/tradingBot/recurring/amend-order-algo");
    assert.equal(getLastCall()?.method, "POST");
    const params = getLastCall()?.params as Record<string, unknown>;
    assert.equal(params.algoId, "123");
    assert.equal(params.stgyName, "New Name");
  });

  it("only sends algoId and stgyName (no extra fields)", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({ algoId: "123", stgyName: "New Name" }, makeContext(client));
    const params = getLastCall()?.params as Record<string, unknown>;
    assert.deepEqual(Object.keys(params).sort(), ["algoId", "stgyName"]);
  });

  it("requires both algoId and stgyName in inputSchema", () => {
    const required = tool.inputSchema.required as string[];
    assert.ok(required.includes("algoId"));
    assert.ok(required.includes("stgyName"));
  });
});

describe("recurring_stop_order", () => {
  const tools = registerRecurringTools();
  const tool = tools.find((t) => t.name === "recurring_stop_order")!;

  it("calls /recurring/stop-order-algo via POST with array body", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({ algoId: "456" }, makeContext(client));
    assert.equal(getLastCall()?.endpoint, "/api/v5/tradingBot/recurring/stop-order-algo");
    assert.equal(getLastCall()?.method, "POST");
  });

  it("wraps algoId in array format without algoOrdType", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({ algoId: "456" }, makeContext(client));
    const params = getLastCall()?.params as unknown[];
    assert.ok(Array.isArray(params));
    assert.equal(params.length, 1);
    assert.deepEqual(params[0], { algoId: "456" });
  });
});

describe("recurring_get_orders", () => {
  const tools = registerRecurringTools();
  const tool = tools.find((t) => t.name === "recurring_get_orders")!;

  it("defaults to pending endpoint when status is omitted", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({}, makeContext(client));
    assert.equal(getLastCall()?.endpoint, "/api/v5/tradingBot/recurring/orders-algo-pending");
  });

  it("uses history endpoint when status=history", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({ status: "history" }, makeContext(client));
    assert.equal(getLastCall()?.endpoint, "/api/v5/tradingBot/recurring/orders-algo-history");
  });

  it("does not send algoOrdType in params", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({}, makeContext(client));
    const params = getLastCall()?.params as Record<string, unknown>;
    assert.equal(params.algoOrdType, undefined);
  });

  it("passes algoId and pagination params", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({ algoId: "789", after: "a1", before: "b1", limit: 50 }, makeContext(client));
    const params = getLastCall()?.params as Record<string, unknown>;
    assert.equal(params.algoId, "789");
    assert.equal(params.after, "a1");
    assert.equal(params.before, "b1");
    assert.equal(params.limit, 50);
  });
});

describe("recurring_get_order_details", () => {
  const tools = registerRecurringTools();
  const tool = tools.find((t) => t.name === "recurring_get_order_details")!;

  it("calls /recurring/orders-algo-details via GET", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({ algoId: "789" }, makeContext(client));
    assert.equal(getLastCall()?.endpoint, "/api/v5/tradingBot/recurring/orders-algo-details");
    assert.equal(getLastCall()?.method, "GET");
  });
...
describe("recurring_get_sub_orders", () => {
  const tools = registerRecurringTools();
  const tool = tools.find((t) => t.name === "recurring_get_sub_orders")!;

  it("calls /recurring/sub-orders via GET", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({ algoId: "789" }, makeContext(client));
    assert.equal(getLastCall()?.endpoint, "/api/v5/tradingBot/recurring/sub-orders");
    assert.equal(getLastCall()?.method, "GET");
  });
  ...
  it("passes ordId and pagination params", async () => {
    const { client, getLastCall } = makeMockClient();
    await tool.handler({ algoId: "789", ordId: "o1", after: "a1", before: "b1", limit: 300 }, makeContext(client));
    const params = getLastCall()?.params as Record<string, unknown>;
    assert.equal(params.ordId, "o1");
    assert.equal(params.after, "a1");
    assert.equal(params.limit, 300);
  });

  it("limit description mentions default 300", () => {
    const props = tool.inputSchema.properties as Record<string, { description: string }>;
    assert.ok(props.limit.description.includes("300"));
  });
});
