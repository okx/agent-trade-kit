...
describe("reResolveDoh", () => {
  it("returns proxy node and writes cache when binary succeeds", async () => {
    const result = await reResolveDoh("proxy.test", "", undefined, cachePath);
    ...
    assert.equal(file["proxy.test"].mode, "proxy");
    assert.equal(file["proxy.test"].node?.ip, "1.1.1.1");
  });

  it("detects direct mode when binary returns node matching hostname", async () => {
    const result = await reResolveDoh("direct.test", "", undefined, cachePath);

    assert.equal(result.mode, "direct");
    assert.equal(result.node, null);

    const file = JSON.parse(readFileSync(cachePath, "utf-8")) as DohCacheFile;
    assert.equal(file["direct.test"].mode, "direct");
  });

  it("returns mode=null and does NOT write cache when binary fails", async () => {
    const result = await reResolveDoh("fail.test", "", undefined, cachePath);
  ...
  it("appends failed IP to exclude list and persists with new node", async () => {
    ...
    await reResolveDoh("multi.test", "", undefined, cachePath);
    const file1 = JSON.parse(readFileSync(cachePath, "utf-8")) as DohCacheFile;
    assert.equal(file1["multi.test"].node?.ip, "1.1.1.1");

    // nodeA fails → exclude 1.1.1.1 → get nodeB (2.2.2.2)
    const result = await reResolveDoh("multi.test", "1.1.1.1", undefined, cachePath);
    assert.equal(result.mode, "proxy");
    assert.equal(result.node?.ip, "2.2.2.2");

    const file2 = JSON.parse(readFileSync(cachePath, "utf-8")) as DohCacheFile;
    assert.equal(file2["multi.test"].node?.ip, "2.2.2.2");
    assert.equal(file2["multi.test"].failedNodes.length, 1);
    assert.equal(file2["multi.test"].failedNodes[0].ip, "1.1.1.1");
  });
  ...
  it("does not duplicate failed IP when already in list", async () => {
    ...
    const result = await reResolveDoh("proxy.test", "1.1.1.1", undefined, cachePath);
    assert.equal(result.mode, "proxy");

    const updated = JSON.parse(readFileSync(cachePath, "utf-8")) as DohCacheFile;
    // Should still be 1, not 2
    assert.equal(updated["proxy.test"].failedNodes.length, 1);
  });
  ...
  it("all nodes exhausted → no cache write, returns null", async () => {
    ...
    const result = await reResolveDoh("multi.test", "2.2.2.2", undefined, cachePath);
    assert.equal(result.mode, null);
    assert.equal(result.node, null);
    ...
    assert.equal(cached["multi.test"].node?.ip, "2.2.2.2");
  });
  ...
  it("expired failed nodes (>1h) are filtered out", async () => {
    ...
    assert.equal(cached["multi.test"].failedNodes.length, 1);
    assert.equal(cached["multi.test"].failedNodes[0].ip, "2.2.2.2");
  });
  ...
  it("empty failedIp does not add phantom entry", async () => {
    ...
    const file = JSON.parse(readFileSync(cachePath, "utf-8")) as DohCacheFile;
    assert.equal(file["proxy.test"].failedNodes.length, 0);
  });
});
