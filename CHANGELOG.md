...
- **`okx-prediction` skill** guides agents through event browsing, account queries (HMAC-auth), live CLOB price / order-book streams, and dry-run-gated trade placement and CTF split/merge/redeem on prediction markets. References split across `data-commands.md`, `account-commands.md`, `clob-commands.md`, `ctf-commands.md`, `stream-commands.md`, and `workflows.md`.
