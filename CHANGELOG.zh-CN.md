...
- **DoH binary 支持 linux-arm64**：`postinstall` 现在会在 ARM Linux 主机上下载 linux-arm64 版本的 `okx-doh-resolver`。

### 修复

- **Skills：移除硬编码中文提示语**：将 `SKILL.md` 中两处硬编码中文澄清提示替换为英文指令式文本，使 agent 能根据用户语言自适应响应。
