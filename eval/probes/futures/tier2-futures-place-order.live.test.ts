...
const USER_PROMPT = `[Automated eval — no human will respond] Use the OKX CLI futures_place_order tool to place a limit buy order for 1 contract of BTC-USD-250926 (quarterly futures) at price 1. The system is in demo/dry-run mode. Report the command and result. Constraints: do NOT start an OAuth login flow, do NOT prompt for credentials, do NOT offer auth-method menus. If the tool fails or auth is unavailable, briefly note the failure and STILL include "${EVAL_NONCE}" verbatim somewhere in your final reply.`;
    ...
    for (let attempt = 1; attempt <= runsPerModel; attempt++) {
      it(`${model} attempt ${attempt}`, async () => {
        ...
        try {
          ...
          const attemptedTool = /futures.*order|delivery.*order|BTC-USD|auth|credential|401|unauthorized|login|API.?key|not.*configured|requires.*authentication/i.test(trace.assistantReply);
