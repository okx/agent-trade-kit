// eval/probes/outcomes/tier2-outcomes-clob-market-order.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-clob-market-order';
const USER_PROMPT = 'Place a market YES order for prediction asset 100888000 in demo/dry-run mode using the okx outcomes clob market-order subcommand. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes setup. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "clob", "market-order"]];
const EXPECTATION = 'okx outcomes clob market-order';
