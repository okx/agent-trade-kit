// eval/probes/outcomes/tier2-outcomes-ctf-split.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-ctf-split';
const USER_PROMPT = 'Split a xp CTF token for prediction asset 100888000 in demo/dry-run mode using the okx outcomes ctf split subcommand. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes setup. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "ctf", "split"]];
const EXPECTATION = 'okx outcomes ctf split';
