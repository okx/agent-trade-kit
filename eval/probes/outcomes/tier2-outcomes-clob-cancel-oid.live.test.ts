// eval/probes/outcomes/tier2-outcomes-clob-cancel-oid.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-clob-cancel-oid';
const USER_PROMPT = 'Cancel prediction order with order id ord_demo_001 in demo/dry-run mode using the okx outcomes clob cancel-oid subcommand. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes setup. Just run the okx outcomes clob cancel-oid command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "clob", "cancel-oid"]];
const EXPECTATION = 'okx outcomes clob cancel-oid';
