// eval/probes/outcomes/tier2-outcomes-clob-heartbeat.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-clob-heartbeat';
const USER_PROMPT = 'Send a heartbeat to keep prediction orders alive in demo/dry-run mode using the okx outcomes clob heartbeat subcommand. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes setup. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "clob", "heartbeat"]];
const EXPECTATION = 'okx outcomes clob heartbeat';
