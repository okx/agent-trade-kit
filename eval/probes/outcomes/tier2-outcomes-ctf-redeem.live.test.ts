// eval/probes/outcomes/tier2-outcomes-ctf-redeem.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-ctf-redeem';
const USER_PROMPT = 'Redeem resolved prediction outcome tokens in demo/dry-run mode using the okx outcomes ctf redeem subcommand. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes setup. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "ctf", "redeem"]];
const EXPECTATION = 'okx outcomes ctf redeem';
