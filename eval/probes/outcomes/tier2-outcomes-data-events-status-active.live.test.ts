// eval/probes/outcomes/tier2-outcomes-data-events-status-active.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-data-events-status-active';
const USER_PROMPT = 'Show me the 10 most active prediction events. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes setup. Just run the okx outcomes data events --status active command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "data", "events", "--status", "active"]];
const EXPECTATION = 'okx outcomes data events --status active';
