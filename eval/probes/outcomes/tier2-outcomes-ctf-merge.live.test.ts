// eval/probes/outcomes/tier2-outcomes-ctf-merge.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-ctf-merge';
const USER_PROMPT = 'Merge YES and NO CTF tokens for prediction asset 100888000 in demo/dry-run mode using the okx outcomes ctf merge subcommand. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes setup. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "ctf", "merge"]];
const EXPECTATION = 'okx outcomes ctf merge';
