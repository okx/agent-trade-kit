// eval/probes/outcomes/tier2-outcomes-account-order.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-account-order';
const USER_PROMPT = 'Get my prediction market order details with order ID ord_demo_001. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes auth login. Just run the okx outcomes account order command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "account", "order"]];
const EXPECTATION = 'okx outcomes account order';
