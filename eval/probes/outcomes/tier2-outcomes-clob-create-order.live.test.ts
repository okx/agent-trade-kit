// eval/probes/outcomes/tier2-outcomes-clob-create-order.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-clob-create-order';
const USER_PROMPT = 'Place a YES limit prediction order for asset 100888000 at price 0.55, size 100 shares in demo/dry-run mode using the okx outcomes clob create-order subcommand. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes setup. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "clob", "create-order"]];
const EXPECTATION = 'okx outcomes clob create-order';
