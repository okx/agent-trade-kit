// eval/probes/outcomes/tier2-outcomes-account-closed-positions.live.test.ts
...
const PROBE_ID = 'tier2.outcomes-account-closed-positions';
const USER_PROMPT = 'Show my closed prediction market positions. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx outcomes auth login. Just run the okx outcomes account positions --status closed command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "outcomes", "account", "positions", "--status", "closed"]];
const EXPECTATION = 'okx outcomes account positions --status closed';
