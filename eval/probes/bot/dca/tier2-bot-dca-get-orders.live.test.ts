// eval/probes/bot/dca/tier2-bot-dca-get-orders.live.test.ts
    ...
    for (let attempt = 1; attempt <= runsPerModel; attempt++) {
      it(`${model} attempt ${attempt}`, async () => {
        ...
        try {
          ...
          const hasDcaContext = /DCA|dollar.*cost|recurring|bot|order|no.*active|empty|auth|credential|401|unauthorized|login|API.?key|not.*configured|requires.*authentication/i.test(trace.assistantReply);
          status = (hasNonce && hasDcaContext) ? 'pass' : 'fail';
          if (!hasNonce) failure_reason = 'nonce missing';
          else if (!hasDcaContext) failure_reason = 'no DCA bot context in reply';
        } catch (e: any) {
          failure_reason = e.message;
          evidence.error = e.message;
        }
        recordResult({
          probe_id: 'tier2.bot-dca-get-orders',
