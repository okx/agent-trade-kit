// eval/probes/earn/savings/tier2-earn-get-fixed-earn-products.live.test.ts
...
const PROBE_ID = 'tier2.earn-get-fixed-earn-products';
const USER_PROMPT = 'Show me all available fixed earn products on OKX. I want to see which ones still have quota. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx config init. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "earn", "savings", "fixed-products"]];
const EXPECTATION = 'okx earn savings fixed-products';
