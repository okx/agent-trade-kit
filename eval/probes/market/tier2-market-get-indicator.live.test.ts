// eval/probes/market/tier2-market-get-indicator.live.test.ts
...
// Plan B: omitting --params now yields values (CLI fills a default
// paramList for period-based indicators), so the prompt asks for an
// indicator without specifying a period.
...
const PROBE_ID = 'tier2.market-get-indicator';
const USER_PROMPT = 'Get the EMA indicator values for BTC-USDT. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx config init. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "market", "indicator", "ema", "BTC-USDT"]];
const EXPECTATION = 'okx market indicator ema BTC-USDT';
