// eval/probes/news/tier2-news-list-calendar-regions.live.test.ts
...
const PROBE_ID = 'tier2.news-list-calendar-regions';
const USER_PROMPT = 'Which countries or regions are available for the economic calendar on OKX? List all supported regions. Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx config init. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "news", "list-regions"]];
const EXPECTATION = 'okx news list-regions';
