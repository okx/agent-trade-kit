...
const USER_PROMPT = `Use the OKX CLI skills_search tool to search for skills related to "market analysis". List at least 2 results with their names and descriptions. Include "${EVAL_NONCE}" verbatim in your reply.`;
    ...
    for (let attempt = 1; attempt <= runsPerModel; attempt++) {
      it(`${model} attempt ${attempt}`, async () => {
        ...
        try {
          ...
          const hasSkillData = /skill|market.*analysis|strategy|result/i.test(trace.assistantReply);
