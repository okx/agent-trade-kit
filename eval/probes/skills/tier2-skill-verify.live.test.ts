// eval/probes/skills/tier2-skill-verify.live.test.ts
// Trace-based probe. Asserts that the agent invoked `okx skill verify <name>`
// via the exec tool. Tool-call intent + parameters is what we measure;
// upstream API auth/401 errors are not a signal.
...
const PROBE_ID = 'tier2.skill-verify';
const USER_PROMPT = 'Use the OKX CLI to verify the signature of an installed skill named "okx-cex-trade". Skip any auth check — assume credentials are configured. Do NOT run okx auth login or okx config init. Just run the appropriate okx CLI command once.';
const EXPECTED_COMMAND_PATTERNS: string[][] = [["okx", "skill", "verify"]];
const EXPECTATION = 'okx skill verify <name>';
