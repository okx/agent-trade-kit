...
> **Safety**: the skill must **never** ask for or pass `--key` directly. Always rely on the env var. If the user pastes a private key in chat, refuse, tell them to revoke it, and walk them through `okx outcomes setup` instead.
...
- **No agent private key configured**: binary returns `NotAuthenticated` — guide the user to `okx outcomes setup`.
