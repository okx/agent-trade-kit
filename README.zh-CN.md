...
| 特性 | 说明 |
|------|------|
| **163 个工具，11 大模块（17 子模块）** | 完整交易生命周期：行情 → 下单 → 算法单 → 账户管理 → 赚币 → 交易机器人 → 事件合约 → 资讯 → 聪明钱信号 |
...
| `prediction` | 0 (wrapper) | OKX 预测市场（YES/NO 事件合约）—— CLI-only 包装器，透传到外部 `okx-predict` 二进制（`@okx/predict-market-cli`）。事件浏览、HMAC 鉴权账户查询、CLOB 价格 / 盘口、dry-run 二段确认下单、CTF 拆分 / 合并 / 赎回 | [→](docs/designs/prediction-wrapper.md) |
...
| [`okx-prediction`](skills/okx-prediction/SKILL.md) | 预测市场：YES/NO 事件合约、CLOB 下单、CTF 拆分 / 合并 / 赎回（需单独安装 `@okx/predict-market-cli` 二进制） | 是 |
