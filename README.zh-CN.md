...
| `outcomes` | 0 (wrapper) | OKX Outcomes（YES/NO 事件合约，原 OKX 预测市场）—— CLI-only 包装器，透传到外部 `okx-outcomes` 二进制（通过 `github.com/okx/outcomes` 的 `install.sh` 安装）。事件浏览、HMAC 鉴权账户查询、CLOB 价格 / 盘口、dry-run 二段确认下单、CTF 拆分 / 合并 / 赎回 | [→](docs/designs/outcomes-wrapper.md) |
